...
class LinkedList(object):
    ...
    #######################################
    ### Mètodes per gestionar la classe ###
    #######################################
    
    # comprueba si está vacía la lista
    def isEmpty(self):
	return len(self) == 0
	...
	''' Crea un array predeterminat de nodes on els valors son números de 1 a n '''
	for i in xrange(0, until):
	    self._head = Node((until - i), self._head) # el __head serà el primer Node de l'array
	    self._size += 1
    ...
    def traverse(self):
	''' Recorre la llista enllaçada imprimint els valors i enllaços '''
	probe = self._head
	while probe:
	...
	''' Remplaça un valor existent per un de nou (busca per valor no per índex) i només la primera coincidència '''
	probe = self._head
	while probe and targetItem != probe.getData():
	    probe = probe.getNext()
	if probe:
	...
	''' Remplaça totes les coincidències existents per un nou valor '''
	probe = self._head
	while probe:
	...
	''' Remplaça un item buscant per l'índex i no pel valor '''
	index -= 1
	...
	self._head = Node(data, self._head) # l'enllaç apunta a l'anterior primer Node
	self._size += 1
	...
	probe = self._head
	if not(probe):
	    self.insertFirst(data) # Si l'array està buit inserta el Node
	else: 
	    while probe.getNext():
		probe = probe.getNext() 
	    probe.setNext(Node(data)) # canvia l'enllaç del penúltim node per apuntar al nou al mateix temps que crea el nou Node
	    self._size += 1
	
    def inserti(self, index, data):
	''' Inserta un Node en qualsevol posició '''
	if index == 0:
	...
	if not self._head.getNext():
	    ...
	    probe = self._head
	    while probe.getNext().getNext(): # han d'existir dos Nodes mes enllà
		probe = probe.getNext() # llavors ja tenim l'element que volem eliminar
	    res = probe.getNext().getData()
	    probe.setNext(None) # i el deixem sense enllaç
	self._size -= 1
	return res
    
    def removei(self, index):
	''' Esborra el node de la ièssima posició i retorna el seu valor '''
	if index == 0:
