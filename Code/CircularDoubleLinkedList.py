...
class CircularDoubleLinkedList(DoubleLinkedList):
    ...
    def traverse(self):
	''' Recorre la llista enllaçada circular imprimint els valors i enllaços '''
	probe = self._head
	i = 0
	while i < len(self):
	...
	''' Remplaça un valor existent per un de nou (busca per valor no per index) i només la primera coincidència '''
	probe = self._head
	i=0
	while i < len(self) and targetItem != probe.getData():
	    probe = probe.getNext()
