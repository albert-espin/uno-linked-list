...
class DoubleLinkedList(LinkedList):
	...
	'''Afegeix two-way-nodes a l'estructura enllaçada '''
    ...
    def traverse(self):
	''' Recorre la llista enllaçda imprimint els valors i enllaços '''
	probe = self._head
	while probe:
	...
	if not self._head:
	    ...
	    self._head = TwoWayNode(data, None, self._head) # l'enllaç apunta a l'anterior primer Node
	...
	probe = self._head
	if not probe:
	    probe = TwoWayNode(data) # Si l'array està buit inserta el Node
	else: 
	    while probe.getNext():
		probe = probe.getNext()
	    probe.setNext(TwoWayNode(data, probe)) # canvia l'enllaç del penúltim node per apuntar al nou al mateix temps que crea el nou Node
	self._size += 1
